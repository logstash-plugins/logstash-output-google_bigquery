...
| <<plugins-{type}s-{plugin}-ignore_insert_ids>> |<<boolean,boolean>>|No
...
[id="plugins-{type}s-{plugin}-ignore_insert_ids"]
===== `ignore_insert_ids`

  * Value type is <<boolean,boolean>>
  * Default value is `true`

BigQuery insertId can be used for de duplication of data.
If true, insertId field is treated as a BigQuery column
If false, insertId field is additionally used for de duplication.
The default value is true.

NOTE: You need to add "insertId" field
